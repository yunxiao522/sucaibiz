...
*/
(function($j){
$j.fn.html5uploader = function(opts){
	...
	var defaults = {
		...
		itemTemplate:'<li id="$j{fileID}file"><div class="progress"><div class="progressbar"></div></div><span class="filename">$j{fileName}</span><span class="progressnum">0/$j{fileSize}</span><a class="uploadbtn">上传</a><a class="delfilebtn">删除</a></li>',//上传队列显示的模板,最外层标签使用<li>
	...
	var option = $j.extend(defaults,opts);
	...
	this.each(function(){
		...
		inputstr += '</a>';
		var fileInputButton = $j(inputstr);
		var uploadFileList = $j('<ul class="filelist"></ul>');
		...
		//创建文件对象
			  ...
			  filter: function(files) {		//选择文件组的过滤方法
				  ...
				  if(!typeArray){
					  ...
					  for(var i in files){
						  if(files[i].constructor==File){
							if($j.inArray(files[i].type,typeArray)>=0){
								arr.push(files[i]);	
								}
							else{
								alert('文件类型不允许！');
			  ...
			  onSelect: option.onSelect||function(files){
				 ...
				 for(var i=0;i<files.length;i++){
					...
					//处理模板中使用的变量
					html = html.replace(/\$j{fileID}/g,file.index).replace(/\$j{fileName}/g,file.name).replace(/\$j{fileSize}/g,formatFileSize(file.size));
				 ...
				 if(!option.auto){
					_this.find('.uploadbtn').die().live('click',function(){
						var index = parseInt($j(this).parents('li').attr('id'));
