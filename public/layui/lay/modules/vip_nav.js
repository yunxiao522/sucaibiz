            ...
            $.get(addr, data, function (res) {
                ...
                if (res.data) {
                    $(res.data).each(function (k, v) {
                        ...
                        if (v.subset) {
                            view += '<a href="javascript:;"><img class="layui-icon" style="width:15px;height:15px;margin-right:5px;" src="' + v.icon + '" />' + v.text + '</a><dl class="layui-nav-child">';
                            $(v.subset).each(function (ko, vo) {
                                ...
                                view += '<img class="layui-icon" style="width:15px;height:15px;margin-right:5px;" src="' + vo.icon + '" />' + vo.text + '</a></dd>';
                            });
                            view += '<dl>';
                        } else {
                            if (v.target) {
