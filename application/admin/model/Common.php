...
class Common extends Base
{
    ...
    //添加操作日志表
    private function insertOperate($data ,$type = 1 ,$class = '',$where){
    ...
    public function updateData($where = [] ,$data = [] ,$class = 'admin' ,$table_name = ''){
        ...
        foreach ($data as $key => $value){
            if($key != 'create_time' &&$key != 'alter_time'){
        ...
        Db::startTrans();
        if(isset($where['id'])){
            $res = Mysql::update($table_name ,'id' ,$where['id'] ,$data);
        }else{
            $res = Db::name($table_name)->where($where)->update($data);
        }
        if($res !== false){
            if($this->insertOperate($log_centent ,1 ,$class,$where)){
                Db::commit();
                return true;
            }else{
                Db::rollback();
